...
def shift_shots_in_sequencer(shots, offset, before=None, after=None):
    ...
    :param list|str shots: representing maya shot nodes.
    :param float offset:
    :param float before: filter shots set before given time in the sequencer timeline.
    :param float after: filter shots set after given time in the sequencer timeline.
    """
    if before:
...
def retime_animation_in_shot_timerange(
        start_frame, end_frame, new_start_frame, new_end_frame, snap_keys=True):
    """
    ...
    :param bool snap_keys: snap scaled keyframes during animation retime.
    """
    shots = filter_shots_from_range(start_frame=start_frame, end_frame=end_frame)
    shots = filter_locked_shots(shots)
    if len(shots) > 1:
        message = (
